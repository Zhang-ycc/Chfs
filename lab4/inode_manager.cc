...
inode_manager::write_file(uint32_t inum, const char *buf, int size)
{
    ...
    if (origin_num > new_num) {
        for (int i = new_num; i < origin_num; ++i) {
            bm->free_block(ino->blocks[i]);
        }
    } else if (origin_num < new_num) {
        for (int i = origin_num; i < new_num; ++i) {
            alloc_new_block(i, ino);
        }
    }
    ...
    if (cursor < size) {
        ...
        bm->write_block(ino->blocks[new_num], buf_tmp);
    }
...
void
inode_manager::alloc_new_block(uint32_t id, inode *ino) {
    if (ino == nullptr)
        return;

    if (id < NDIRECT)
        ino->blocks[id] = bm->alloc_block();
    else if (id < MAXFILE) {
        ...
        ((blockid_t *) buf)[id - NDIRECT] = bm->alloc_block();
